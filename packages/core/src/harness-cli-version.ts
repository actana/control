...
import { resolveHarnessCliUpdateCommands } from "./harness-cli-version-requirements";
import { compareCliVersions, extractCliVersion } from "@actana/shared/harness-cli-version-compare";
import { buildCmdScriptCommand, isWindowsCommandScript } from "@actana/shared/windows-cmd";

export { compareCliVersions, extractCliVersion } from "@actana/shared/harness-cli-version-compare";
...
function baseCheckFields(
  ...
  return {
    ...
    updateCommands: resolveHarnessCliUpdateCommands(requirement.updateCommands, platform),
  };
}
...
export function checkHarnessCliVersion(
  binary: string,
  env: NodeJS.ProcessEnv,
  requirement: HarnessCliVersionRequirement,
  platform: NodeJS.Platform = os.platform(),
...
// uncached probe blocks ALL windows for its full duration (the Harness is a
...
const versionCheckCache = new Map<string, { check: HarnessVersionCheck; at: number }>();
const VERSION_CHECK_FAILURE_TTL_MS = 30_000;

export function checkHarnessCliVersionCached(
  binary: string,
  env: NodeJS.ProcessEnv,
  requirement: HarnessCliVersionRequirement,
  platform: NodeJS.Platform = os.platform(),
  ...
  const check = checkHarnessCliVersion(binary, env, requirement, platform);
  versionCheckCache.set(key, { check, at: Date.now() });
  return check;
}

export function clearHarnessCliVersionCache(): void {
  versionCheckCache.clear();
}

export function harnessVersionErrorMessage(check: Exclude<HarnessVersionCheck, { ok: true }>): string {
