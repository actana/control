// `core-manifest.json` — the tarball's self-description, written by
// `scripts/build-core-tarball.mjs` at the tarball root.
//
 ...
 * every one of those means "this directory is not an extracted Core
 * tarball", which is the only distinction a caller needs to make.
 */
export function readCoreManifest(installRoot: string): CoreManifest | null {
  let parsed: unknown;
  try {
