...
function harnessSpecificHomePathCandidates(home: string, platform: NodeJS.Platform): string[] {
  return harnessHomePathSuffixes(platform).map((suffix) => path.join(home, ...suffix.split("/")));
}
...
function windowsPathCandidates(home: string, env: NodeJS.ProcessEnv): Array<string | undefined> {
  ...
  return [
    ...harnessSpecificHomePathCandidates(home, "win32"),
...
function posixPathCandidates(home: string, env: NodeJS.ProcessEnv): string[] {
  ...
  return [
    ...harnessSpecificHomePathCandidates(home, os.platform() === "darwin" ? "darwin" : "linux"),
    path.join(home, ".local", "bin"),
    path.join(home, "bin"),
    path.join(home, ".cargo", "bin"),
    path.join(home, ".bun", "bin"),
