...
import { _setHarnessAccountsDepsForTests, readHarnessAccounts } from "../harness-accounts";
...
describe("readHarnessAccounts", () => {
  ...
  it("surfaces the Codex account id without any token material", () => {
    _setHarnessAccountsDepsForTests({ codexReader: () => ({ accountId: "acct_123" }) });
  ...
  it("surfaces the Cursor user id", () => {
    _setHarnessAccountsDepsForTests({ cursorReader: () => "user_abc123" });
  ...
  it("never includes token-shaped fields in the payload", () => {
    ...
    const serialized = JSON.stringify(readHarnessAccounts()).toLowerCase();
