// The blob registry on disk — one blob per named Core (#129 D9).
//
...
// two things.
//
// **Why the on-disk half lives here rather than in `packages/cli`.** It used to
// live there in full, and the client half of `actana` is still its main
// caller — `packages/cli/src/blob-registry.ts` re-exports every name below and
// adds the part that needs the SDK: reading an entry back as a `core ls` row.
// The daemon needs the *writing* half too. A containerised Core has no `actana
// setup` to do the wiring for it (ADR 0016 D13), so it registers itself at boot
// through `local-core-wiring.ts` beside this file, and a daemon that wrote the
// registry through its own second implementation would be two programs
// disagreeing about where a credential lives on one machine.
//
