...
import { queryProjects, queryTasks, type HarnessQuerySqlite } from "../harness-query";

// Pure SQL helpers that write to the Harness's projects + tasks tables and
...
function asWriter(db: Database.Database): HarnessMutationSqlite {
  return db as unknown as HarnessMutationSqlite;
}

function asReader(db: Database.Database): HarnessQuerySqlite {
  return db as unknown as HarnessQuerySqlite;
}
