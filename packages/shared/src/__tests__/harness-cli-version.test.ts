...
describe("Harness version helpers", () => {
  ...
  if (process.platform === "win32") {
    it("executes Windows command shims with verbatim cmd.exe arguments", () => {
      ...
      const result = checkHarnessCliVersion(
        binary,
        {
  ...
  it("compares Cursor calendar versions by date because the build hash is not orderable", () => {
    const cursorRequirement = HARNESS_CLI_CONFIG["cursor-cli"];
  ...
  it("returns platform-specific Cursor install commands", () => {
    ...
    expect(resolveHarnessCliUpdateCommands(cursorRequirement.updateCommands, "win32")).toEqual([
      "irm 'https://cursor.com/install?win32=true' | iex",
      "agent update",
    ]);
    expect(resolveHarnessCliUpdateCommands(cursorRequirement.updateCommands, "darwin")).toEqual([
  ...
  it("filters brew-only Codex update commands on Windows", () => {
    const codexRequirement = HARNESS_CLI_CONFIG.codex;

    expect(resolveHarnessCliUpdateCommands(codexRequirement.updateCommands, "win32")).toEqual([
      "npm install -g @openai/codex@latest",
    ]);
    expect(resolveHarnessCliUpdateCommands(codexRequirement.updateCommands, "darwin")).toEqual([
      "npm install -g @openai/codex@latest",
      "brew upgrade codex",
    ]);
  });
  ...
  if (os.platform() !== "win32") {
    it("caches a passing version probe so repeat spawns skip the CLI launch", () => {
      ...
      try {
        ...
        fs.writeFileSync(
          binary,
          `#!/bin/sh\necho probe >> "${counter}"\necho "${HARNESS_CLI_CONFIG["claude-code"].minimumVersion} (Claude Code)"\n`,
          { mode: 0o755 },
        );

        clearHarnessCliVersionCache();
        const env = { PATH: process.env.PATH ?? "", HOME: process.env.HOME ?? "" };
        const first = checkHarnessCliVersionCached(
          binary,
          env,
          HARNESS_CLI_CONFIG["claude-code"],
          "darwin",
        );
        const second = checkHarnessCliVersionCached(
          binary,
          env,
          HARNESS_CLI_CONFIG["claude-code"],
          "darwin",
    ...
    it("re-probes past a passing cache entry when fresh is requested", () => {
      ...
      try {
        ...
        const first = checkHarnessCliVersionCached(binary, env, requirement, "darwin");
        const fresh = checkHarnessCliVersionCached(binary, env, requirement, "darwin", {
          fresh: true,
        });
        // The fresh result replaces the cache entry for later cached reads.
        const third = checkHarnessCliVersionCached(binary, env, requirement, "darwin");
