// Pure SQL helpers that mutate the Harness's projects + tasks tables and read
// the derived sessions view for the write path (issue 04, ADR 0004).
//
// The Harness process is the sole VM-side writer of the shared SQLite (ADR
// 0004); on remote Cores no sibling stateful server exists, so
// `PtyCoreLinkServer` dispatches `projectsMutate` / `tasksMutate` /
// `sessionsList` frames to a `HarnessMutationPort` whose real implementation
// (packages/harness/src/harness-mutation-store.ts) opens `missioncontrol.db` read-write
...
// minimal `HarnessMutationSqlite` interface so tests can pass an in-memory
 ...
 */
export interface HarnessMutationSqlite {
  prepare(sql: string): {
 ...
 * stays SQL-only (no dependency on `PtyHarnessCore`).
 */
 ...
 *    both node and the browser; the Harness caller passes the real
...
export function validateProjectPath(rawPath: string, probe: ProjectPathProbe): string {
  ...
  if (!probe.isAbsolute(trimmed)) {
    throw new Error(`project path must be absolute on the Harness's machine (got: ${trimmed})`);
  }
  ...
  if (!stat) {
    throw new Error(`project path does not exist on the Harness: ${trimmed}`);
  }
  if (!stat.isDirectory()) {
 ...
 * when the row is missing. Pin state is a Harness fact — two Panels
 ...
 * Panel wants optimistic-UI parity, else generated on the Harness.
 */
 ...
 * string. The Harness does not validate against `SESSION_ICON_OPTIONS` — the
 * Panel-side `isSessionIcon` guard falls back to `DEFAULT_SESSION_ICON` when a
 * row carries an unknown id, so a Harness on a newer version can hand a Panel
 ...
 * (if the Harness's PTY core currently has one for that task), so a
