// Pure SQL helpers that read the Harness's projects + tasks tables and map
// them to core-link snapshots for the per-Core navigation + Fleet view (issue
// 07, ADR 0001).
//
// The Harness is the single source of truth for projects and tasks; the Panel
// holds none. The `projectsList` / `tasksList` core-link frames delegate to a
// `HarnessQueryPort` whose real implementation (packages/harness/src/harness-query-store.ts)
...
// minimal `HarnessQuerySqlite` interface so tests can pass an in-memory
 ...
 */
export interface HarnessQuerySqlite {
  prepare(sql: string): {
...
/**
 * Read every project on this Harness as a flattened snapshot. Returns an empty
 * array when the table is absent (a Harness whose DB hasn't bootstrapped yet)
 * or when there are no rows. `path` is a VM path — only the Harness can
 * validate it (CONTEXT.md "Project").
 */
export function queryProjects(sqlite: HarnessQuerySqlite): CoreLinkProjectSnapshot[] {
  let rows: ProjectRow[];
  try {
    rows = sqlite
      .prepare(
...
/**
 * Read every active (non-archived) task on this Harness, optionally filtered
