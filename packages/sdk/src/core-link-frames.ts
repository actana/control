...
/**
 * The kind of the event the Core appends when it **gives up** delivering a
 * Session's starting prompt (#483).
 *
 * The Session already moves to `needs-input` when this happens, and that is
 * what a client renders. What the status cannot say is *why*, and the why is
 * the difference between "the harness stopped to ask a question" — the ordinary
 * reading of `needs-input`, and the one a script answers with `session send` —
 * and "the prompt never reached the harness at all, so there is no question and
 * no turn". A caller that waited on a start and got a settled Session with no
 * report is owed the second sentence, and until this event there was nowhere it
 * could come from: the giving-up was a line in the Core's own process log.
 *
 ...
 * its own vocabulary (a dialog id it knows, or a composer that never came), not
