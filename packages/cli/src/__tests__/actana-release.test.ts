...
describe("parsing SHA256SUMS", () => {
  ...
  it("reads both the coreutils and the shasum binary-mode spellings", () => {
    const sums = parseShasums(
      `${digest}  actana-core-1.0.0-linux-x64.tar.gz\n` +
        `${"b".repeat(64)} *actana-core-1.0.0-mac-arm64.tar.gz\n`,
    );
    expect(sums.get("actana-core-1.0.0-linux-x64.tar.gz")).toBe(digest);
    expect(sums.get("actana-core-1.0.0-mac-arm64.tar.gz")).toBe("b".repeat(64));
  });
