// Harness build — bundles the daemon + the `actana` CLI entries with esbuild.
//
...
// `node dist/harness-entry.cjs` runs it on the standard Node ABI. Native and
...
await build({
  ...shared,
  entryPoints: ["src/harness-entry.ts"],
  outfile: "dist/harness-entry.cjs",
});

// The `actana` CLI. Deliberately a separate bundle from the daemon: it
// `require`s dist/harness-entry.cjs at runtime for the `daemon` verb rather
// than bundling it, so the tarball ships one copy of the Harness, not two.
await build({
  ...shared,
