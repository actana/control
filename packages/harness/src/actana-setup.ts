...
import { offerAgentInstalls, type AgentInstallOutcome } from "./actana-agents";
...
import type { HarnessManifest } from "./actana-manifest";
...
export type SetupOptions = {
  ...
  sourceRoot: string;
  /** That tree's `harness-manifest.json`. */
  ...
  requestedAgents: readonly TaskAgent[];
  /** `--no-agents` — install nothing and offer nothing. */
  noAgents: boolean;
  /** The Harness's own PATH probe — the source of truth for what is missing. */
...
export type SetupResult = {
  ...
  servicePath: string;
  /** What the service is called: `actana-harness.service`, `com.actana.harness`. */
  serviceName: string;
  ...
  listening: boolean;
  /** What became of each managed agent CLI during the offer round. */
  agents: AgentInstallOutcome[];
};
...
export async function runActanaSetup(opts: SetupOptions): Promise<SetupResult> {
  ...
  if (missing) {
    throw new Error(`${opts.sourceRoot} is not an extracted Harness tarball: no ${missing}`);
  }
  ...
  if (replacingTree && service.isActive()) {
    opts.out("Stopping the running Harness before upgrading it…");
    service.stop();
  }
  ...
  service.install({
    ...
    environment: {
      ...
      AC_HARNESS_PUBLIC_HOST: opts.publicHost,
      AC_USER_DATA_DIR: layout.dataDir,
      AC_HARNESS_MATERIAL_FILE: materialFilePath(layout.configDir),
    },
  });
  ...
  const agents = await offerAgentInstalls({
    availability: opts.probeAgents(),
    requested: opts.requestedAgents,
