// Browsing the Harness's own filesystem, for the Panel's folder picker.
//
// A Project's path is a VM path: only the Harness can say whether it exists,
...
import type { CoreLinkDirEntry, CoreLinkDirListing } from "../../shared/src/core-link-frames";
...
export type DirectoryBrowseOptions = {
  /** The Harness's home directory. Injectable so tests need no real $HOME. */
  home?: string;
...
/**
 * List the directories inside `requested`, or inside the Harness's home when
