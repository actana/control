...
/** What the machine's init system says about the Harness's auto-start unit. */
  ...
  async function ensureLinger(context: PersistenceContext): Promise<boolean> {
    ...
    if (show.status !== 0) {
      context.out(
        "Warning: could not read the linger setting (no logind on this machine?). " +
          "The Harness may stop when you log out.",
      );
      return false;
    }
    ...
    if (context.interactive && !context.assumeYes) {
      const yes = await system.confirm(
        "Enable lingering so the Harness keeps running after you log out?",
        true,
      );
      if (!yes) {
        context.out(
          `Skipped lingering. The Harness will stop when you log out — enable it later with ` +
    ...
    if (system.run("loginctl", ["enable-linger", user]).status !== 0) {
      context.out(
        `Warning: could not enable lingering, so the Harness will stop when you log out. ` +
