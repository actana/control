...
async function verifyAndUnpack(
  ...
  target: string,
  workDir: string,
): Promise<{ root: string; manifest: HarnessManifest }> {
  ...
  if (actual !== expected) {
    throw new Error(
      `checksum mismatch for ${asset} — refusing to install it.\n` +
        `  expected ${expected}\n` +
        `  actual   ${actual}\n` +
        "Nothing was changed; the Harness is still running the version it was. Retry the " +
  ...
  const root = path.join(workDir, `actana-harness-${version}-${target}`);
  if (!fs.existsSync(root)) {
    throw new Error(`${asset} does not contain actana-harness-${version}-${target} — the release asset looks wrong.`);
  }
  const missing = missingTreeFile(root);
  if (missing) {
    throw new Error(`${asset} is not a complete Harness build: no ${missing}.`);
  }

  const manifest = readHarnessManifest(root);
  if (!manifest) throw new Error(`${asset} has no readable harness-manifest.json.`);
...
export async function runActanaUpdate(opts: UpdateOptions): Promise<UpdateResult> {
  ...
  if (!target) {
    throw new Error(
      `there is no Harness build for ${opts.platform}/${opts.arch} — Cores run on macOS and ` +
  ...
  if (version === config.version && missingTreeFile(installDir) === null) {
    opts.out(`Already running Harness ${version} — nothing to update.`);
    return {
      updated: false,
      version,
