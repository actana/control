...
import { ensureStatuslineTap } from "../../shared/src/statusline-tap";
...
import { MAX_TCP_PORT } from "../../shared/src/tcp-port";
import { shortId } from "../../shared/src/short-id";
import {
  resolveSpawnPlan,
  SpawnPolicyError,
  type SpawnRequest,
} from "../../shared/src/pty-spawn-policy";
...
import { applyAgentPtyEnv } from "../../shared/src/agent-pty-env";
import { acquireSpawnSlot, SPAWN_SETTLE_MS } from "./pty-spawn-queue";

function sanitizeEnv(): Record<string, string> {
  const out = sanitizedProcessEnv();
  // The PTY is xterm.js, not whichever terminal launched the Harness. Leaking
...
type Pty = {
  ...
  shell: boolean;
  /**
   * True for VM Shell Sessions (issue 06) — a free-form shell on this Harness's
...
/**
 * Dependencies the PTY harness needs from its host process.
 */
export type PtyHarnessDeps = {
...
/** Event emitted by the harness core for a PTY — `data` (output) or `exit`. */
export type PtyHarnessEvent =
  | { type: "data"; ptyId: string; data: string; seq: number }
  | { type: "exit"; ptyId: string; exitCode: number; signal?: number };

// Module-level state: one harness core per process (the Harness daemon
// OR the standalone Harness process). Set by registerPtyHandlers or
// PtyHarnessCore's constructor.
let activeDeps: PtyHarnessDeps | null = null;
let emitTarget: ((event: PtyHarnessEvent) => void) | null = null;
...
function sendToEmitTarget(event: PtyHarnessEvent): void {
  emitTarget?.(event);
}
...
// spawn/write/resize/kill/replay logic. The standalone Harness process wires it
// to a WebSocket server via PtyCoreLinkServer.

let activeCore: PtyHarnessCore | null = null;

/** Sets the process-wide core that `killAllPtys` delegates to. */
export function setActivePtyHarnessCore(core: PtyHarnessCore | null): void {
  activeCore = core;
}

export class PtyHarnessCore {
  constructor(private readonly deps: PtyHarnessDeps) {
    activeDeps = deps;
  }

  /** Set the sink for data/exit events. Pass null to stop emitting (e.g. on WS disconnect). */
  setEmitTarget(fn: ((event: PtyHarnessEvent) => void) | null): void {
    emitTarget = fn;
  }

  async spawn(opts: SpawnRequest): Promise<{ ptyId: string }> {
    // Agent cold-boots are throttled; shells are not. A login shell is cheap
  ...
  private async spawnUnthrottled(
    ...
    // VM shell has no project folder, and the Harness's own home is the only
    ...
    // Agent-workspace-only setup; a VM Shell Session (and a plain user shell)
