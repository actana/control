// Harness material store — persists cert material + bearer secret to disk so
...
/**
 * The persisted Harness material — everything the daemon needs to restart
 * with the same identity (same CA, same certs, same bearer secret + coreId).
 * Written by `harness install`; read by `harness-entry.ts` on daemon boot when
 * `AC_HARNESS_MATERIAL_FILE` is set.
 */
 ...
 * Used by `harness-entry.ts` which receives the full path via
...
export function loadMaterialFromFile(filePath: string): PersistedMaterial | null {
  ...
  try {
    ...
    log.warn("harness-material.load: corrupt JSON", { filePath });
    return null;
  }
  ...
  if (
    ...
    log.warn("harness-material.load: missing or wrong-typed fields", { filePath });
    return null;
  }
  return {
    caCert: o.caCert,
