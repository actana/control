...
const config: ActanaPlistConfig = {
  ...
  logPath: "/Users/op/Library/Logs/Actana/harness.log",
};
...
describe("renderActanaPlist", () => {
  ...
  it("sends both streams to one log file so `actana logs` has one thing to tail", () => {
    ...
    expect(plist).toContain(
      "<key>StandardErrorPath</key>\n    <string>/Users/op/Library/Logs/Actana/harness.log</string>",
    );
    expect(plist).toContain(
  ...
  it("emits every environment entry sorted, so a re-run rewrites nothing", () => {
    ...
    expect(keys).toEqual(["AC_CORE_LINK_PORT", "AC_HARNESS_REMOTE", "AC_USER_DATA_DIR"]);
  });
...
describe("parseLaunchctlPrint", () => {
  const RUNNING = `com.actana.harness = {
	active count = 1
	path = /Users/op/Library/LaunchAgents/com.actana.harness.plist
	state = running
  ...
  it("reports a loaded-but-not-running job with no pid", () => {
    expect(
      parseLaunchctlPrint(`com.actana.harness = {
	state = waiting
	runs = 3
}`),
...
describe("serviceTarget", () => {
  it("names a job inside its domain", () => {
    expect(serviceTarget("gui/501", "com.actana.harness")).toBe("gui/501/com.actana.harness");
  });
});
...
describe("launchdLogPath", () => {
  it("puts the daemon's output where macOS keeps a user's logs", () => {
    expect(launchdLogPath("/Users/op")).toBe("/Users/op/Library/Logs/Actana/harness.log");
  });
});
