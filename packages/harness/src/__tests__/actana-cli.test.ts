...
import { decodeRegistrationBlob } from "../../../shared/src/registration-blob";
...
import { materialFilePath } from "../harness-material-store";
...
function makeTarballTree(root: string, manifest = MANIFEST): void {
  ...
  fs.writeFileSync(path.join(root, "app", "harness-entry.cjs"), "// daemon\n");
...
function deps(argv: string[], system: ActanaSystem, over: Partial<ActanaCliDeps> = {}): ActanaCliDeps {
  return {
    ...
    probeAgents: () => ({ claude: { status: "available", version: "2.1.0" } }),
    runDaemon: async () => {
      daemonRuns += 1;
    },
    ...over,
...
beforeEach(() => {
  ...
  installRoot = path.join(tmp, "extract", "actana-harness-0.49.0-linux-x64");
...
describe("token regenerate", () => {
  ...
  it("restarts the daemon — until it reloads, the old credentials still work", async () => {
    ...
    expect(system.calls.map((c) => c.join(" "))).toContain(
      "systemctl --user restart actana-harness.service",
    );
  });
...
describe("uninstall", () => {
  it("stops the daemon and leaves no unit, launcher, or install files", async () => {
    ...
    expect(commands).toContain("systemctl --user stop actana-harness.service");
    expect(commands).toContain("systemctl --user disable actana-harness.service");
...
describe("start / stop / restart", () => {
  it.each(["start", "stop", "restart"])("drives systemctl --user %s", async (verb) => {
    ...
    expect(system.calls.map((c) => c.join(" "))).toContain(
      `systemctl --user ${verb} actana-harness.service`,
    );
  });
...
describe("logs", () => {
  it("shows the daemon's journal without a pager", async () => {
    ...
    expect(call.join(" ")).toContain("-u actana-harness.service");
  });
...
describe("macOS", () => {
  ...
  beforeEach(() => {
    macRoot = path.join(tmp, "extract", "actana-harness-0.49.0-mac-arm64");
  ...
  const plistPath = () => path.join(home, "Library", "LaunchAgents", "com.actana.harness.plist");
  ...
  it("installs a LaunchAgent and prints a pairing token", async () => {
    ...
    const text = out.join("\n");
    expect(text).toMatch(/pairing token/i);
    expect(text).toContain("com.actana.harness");
  ...
  it("starts a loaded-but-dead agent — loaded is not the same as running", async () => {
    ...
    expect(await runActanaCli(macDeps(["start"], system))).toBe(0);
    expect(system.calls.map((c) => c.join(" "))).toContain(
      "launchctl kickstart -k gui/501/com.actana.harness",
    );
  });
  ...
  it("tails the LaunchAgent's log file — launchd has no journal", async () => {
    ...
    expect(call.join(" ")).toBe(
      `tail -n +1 ${path.join(home, "Library", "Logs", "Actana", "harness.log")}`,
    );
  ...
  it("creates the log file at setup so the first `logs` is not an error", async () => {
    await macSetup(fakeSystem());
    expect(fs.existsSync(path.join(home, "Library", "Logs", "Actana", "harness.log"))).toBe(true);
  });
...
describe("daemon", () => {
  it("runs the Harness in the foreground — this is what the unit execs", async () => {
...
// ─── agent CLI detection and offers (installer issue 05) ────────────────────
...
describe("agent offers during setup", () => {
  it("installs nothing and prompts for nothing without a terminal", async () => {
    const system = fakeSystem();
    expect(await runActanaCli(deps(["setup"], system, { probeAgents: () => NO_AGENTS }))).toBe(0);

    expect(vendorCommands(system)).toEqual([]);
    expect(out.join("\n")).toContain("actana agents install");
  });

  it("installs exactly the agents named with --with-<agent>", async () => {
    const system = fakeSystem();
    expect(
      await runActanaCli(deps(["setup", "--with-opencode"], system, { probeAgents: () => NO_AGENTS })),
    ).toBe(0);
  ...
  it("accepts the CLI command as the flag name too", async () => {
    const system = fakeSystem();
    await runActanaCli(deps(["setup", "--with-claude"], system, { probeAgents: () => NO_AGENTS }));
  ...
  it("installs every missing agent under --yes", async () => {
    const system = fakeSystem();
    await runActanaCli(deps(["setup", "--yes"], system, { probeAgents: () => NO_AGENTS }));
    expect(vendorCommands(system)).toHaveLength(4);
  });

  it("installs nothing under --no-agents", async () => {
    const system = fakeSystem();
    await runActanaCli(
      deps(["setup", "--yes", "--no-agents"], system, { probeAgents: () => NO_AGENTS }),
    );
    expect(vendorCommands(system)).toEqual([]);
  });

  it("rejects --no-agents together with --with-<agent>", async () => {
    const system = fakeSystem();
    expect(
      await runActanaCli(deps(["setup", "--no-agents", "--with-codex"], system)),
    ).toBe(EXIT_USAGE);
    expect(err.join("\n")).toContain("--no-agents cannot be combined");
  ...
  it("offers each missing agent on a terminal", async () => {
    ...
    await runActanaCli(
      deps(["setup"], system, { interactive: true, probeAgents: () => NO_AGENTS }),
    );
    expect(vendorCommands(system)).toEqual(["curl -fsSL https://claude.ai/install.sh | bash"]);
  });
...
describe("actana agents install", () => {
  it("installs the named agent with the vendor's own installer", async () => {
    ...
    expect(
      await runActanaCli(
        deps(["agents", "install", "opencode"], system, { probeAgents: () => NO_AGENTS }),
      ),
    ).toBe(0);
    expect(vendorCommands(system)).toEqual(["curl -fsSL https://opencode.ai/install | bash"]);
  });
  ...
  it("nudges the running daemon so a paired Panel sees the new agent", async () => {
    ...
    await runActanaCli(
      deps(["agents", "install", "codex"], system, { probeAgents: () => NO_AGENTS }),
    );
  ...
  it("says so when the agent is already installed, and installs nothing", async () => {
    ...
    expect(
      await runActanaCli(
        deps(["agents", "install", "codex"], system, {
          probeAgents: () => ({ ...NO_AGENTS, codex: { status: "available", path: "/usr/bin/codex" } }),
        }),
      ),
    ).toBe(0);
    expect(vendorCommands(system)).toEqual([]);
  ...
  it("exits non-zero when the vendor installer fails", async () => {
    ...
    expect(
      await runActanaCli(
        deps(["agents", "install", "cursor-cli"], system, { probeAgents: () => NO_AGENTS }),
      ),
    ).toBe(1);
  ...
  it("answers an unknown id with the supported list", async () => {
    expect(
      await runActanaCli(deps(["agents", "install", "gemini"], fakeSystem())),
    ).toBe(EXIT_USAGE);
  ...
  it("needs an id, and needs a subcommand", async () => {
    expect(await runActanaCli(deps(["agents", "install"], fakeSystem()))).toBe(EXIT_USAGE);
    expect(await runActanaCli(deps(["agents"], fakeSystem()))).toBe(EXIT_USAGE);
    expect(await runActanaCli(deps(["agents", "list"], fakeSystem()))).toBe(EXIT_USAGE);
    expect(err.join("\n")).toContain("unknown subcommand: list");
  });

  it("is advertised in help", async () => {
    await runActanaCli(deps(["--help"], fakeSystem()));
    expect(out.join("\n")).toContain("agents install");
    expect(out.join("\n")).toContain("--no-agents");
  });
});
