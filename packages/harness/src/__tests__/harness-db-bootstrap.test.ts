...
// Issue 02 — on a harness-only VM (ADR 0003 install, AC_HARNESS_REMOTE=1), the
// Harness process itself owns bringing up `missioncontrol.db` before the
...
function tmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "mc-harness-db-boot-"));
}

describe("bootstrapHarnessDb", () => {
  ...
  it("creates missioncontrol.db and the projects schema on a fresh VM", () => {
    ...
    const result = bootstrapHarnessDb(userDataDir);
  ...
  it("is idempotent — a second call against an already-migrated DB is a no-op", () => {
    const first = bootstrapHarnessDb(userDataDir);
    ...
    const second = bootstrapHarnessDb(userDataDir);
  ...
  it("enables WAL so the two-writer event-log pattern stays valid", () => {
    ...
    // Harness and the (loopback) stateful server both write to the same
  ...
  it("creates the shared event_log table so the Harness can append PTY events", () => {
    // The Harness's PTY spawn/exit events replay through this table; issue 07's
  ...
  it("throws when userDataDir is empty (no silent degradation)", () => {
    ...
    expect(() => bootstrapHarnessDb("")).toThrow();
  });
  ...
  it("throws when the DB can't be opened (parent path is a file)", () => {
    ...
    // as a boot failure, not a broken-but-listening Harness.
    const filePath = path.join(userDataDir, "not-a-dir");
    fs.writeFileSync(filePath, "");
    expect(() => bootstrapHarnessDb(filePath)).toThrow();
  });
});
