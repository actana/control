...
// Harness's machine with NO project-root requirement (a VM shell has no
// project folder). The policy must skip cwd/project-root validation entirely
// and resolve to a login shell — the Harness's own home dir is supplied by
...
describe("resolveSpawnPlan — VM shell sessions (shellSession: true)", () => {
  ...
  it("accepts the Harness's own home dir as cwd (handler-supplied, not renderer)", () => {
    // The handler (PtyHarnessCore.spawn) replaces the empty cwd with its own
    // os.homedir() before calling resolveSpawnPlan, so the plan's cwd is the
    // real home path on the Harness machine — never a renderer-supplied path.
    const plan = resolveSpawnPlan(
