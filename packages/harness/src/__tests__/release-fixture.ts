...
/** Write the file tree a Harness tarball contains, at `root`. */
export function writeTarballTree(root: string, manifest: FixtureManifest): void {
  ...
  fs.writeFileSync(path.join(root, "app", "harness-entry.cjs"), `// daemon ${manifest.version}\n`);
...
function parseAssetName(name: string): { version: string; target: string } | null {
  const match = /^actana-harness-(.+)-([a-z0-9]+-[a-z0-9]+)\.tar\.gz$/.exec(name);
