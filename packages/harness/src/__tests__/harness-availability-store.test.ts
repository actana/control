...
import { AGENTS_AVAILABILITY_EVENT_KIND } from "../../../shared/src/core-link-frames";
import { UI_AGENTS } from "../../../shared/src/agents";
...
// Issue 11: the Harness-side probe publishes CLI availability as (a) a live
...
describe("HarnessAvailabilityStore", () => {
  it("emits one agents:availabilityChanged event on the first probe", () => {
    ...
    expect(kind).toBe(AGENTS_AVAILABILITY_EVENT_KIND);
  ...
  it("re-emits when one agent's availability changes between ticks", () => {
    const appendEvent: ReturnType<typeof vi.fn<AppendEventFn>> = vi.fn(() => 1);
    const first: Record<TaskAgent, ReturnType<typeof mkEntry>> = Object.fromEntries(
      UI_AGENTS.map((a) => [a, mkEntry("available", "/x")]),
    ) as Record<TaskAgent, ReturnType<typeof mkEntry>>;
    const second = { ...first, [UI_AGENTS[0]]: mkEntry("missing") };
    let round = 0;
    const store = new HarnessAvailabilityStore({
      appendEvent,
      tickMs: 60_000,
    ...
    expect(second_payload.availability[UI_AGENTS[0]].status).toBe("missing");
  });
  ...
  it("surfaces a probe exception as a `missing` entry without crashing the tick", () => {
    ...
    const store = new HarnessAvailabilityStore({
      ...
      tickMs: 60_000,
      probe: (agent) => {
        if (agent === UI_AGENTS[0]) throw new Error("boom");
        return { status: "available", path: "/x" };
      },
    });
    store.runProbe();
    expect(store.snapshot()[UI_AGENTS[0]]).toEqual({
      status: "missing",
      reason: "boom",
    });
    expect(store.snapshot()[UI_AGENTS[1]]?.status).toBe("available");
  });
});
