...
import { readHarnessManifest } from "../actana-manifest";
...
describe("landing a newer release", () => {
  ...
  it("installs it beside the old one and repoints `current` at it", async () => {
    ...
    expect(readHarnessManifest(installDirFor(layout, "0.50.0"))?.version).toBe("0.50.0");
  });
...
describe("releases this machine cannot use", () => {
  ...
  it("rejects a tarball that is not a complete Harness build", async () => {
    writeRelease({
      dir: releaseDir,
      version: "0.50.0",
      target: TARGET,
