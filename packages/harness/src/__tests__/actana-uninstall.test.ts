...
function fakeService() {
  const service = {
    ...
    name: "actana-harness.service",
    filePath: path.join(home, ".config", "systemd", "user", "actana-harness.service"),
    isActive: () => true,
    stop() {},
    install() {},
    uninstall() {
...
function installedMachine(): void {
  ...
  fs.writeFileSync(path.join(layout.dataDir, "harness.db"), "sessions and events");
...
describe("uninstall", () => {
  ...
  it("keeps the data dir by default — a reinstall finds its sessions again", () => {
    uninstall();
    expect(fs.readFileSync(path.join(layout.dataDir, "harness.db"), "utf8")).toBe(
      "sessions and events",
    );
  });
