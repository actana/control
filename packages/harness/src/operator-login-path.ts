// Keeping the agent CLIs Actana installed on the operator's login PATH.
//
// `actana agents install <id>` shells to the vendor's own installer, and each
...
import { agentHomePathSuffixes } from "../../shared/src/agent-cli-config";
...
export function renderManagedBlock(suffixes: readonly string[]): string {
  ...
  return [
    MANAGED_BLOCK_BEGIN,
    "# Added by `actana agents install`. Edit above or below, not inside:",
...
/**
 * Put every declared agent CLI directory on the operator's login PATH.
 *
...
export function ensureOperatorLoginPathOnDisk(opts: {
  ...
  return ensureOperatorLoginPath({
    ...
    suffixes: opts.suffixes ?? agentHomePathSuffixes(platform),
