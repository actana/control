// Harness-side event-log store — a read-write handle to the shared SQLite
// `event_log` table, owned by the Harness (PTY-manager) process.
//
// The Harness appends PTY lifecycle events (pty:spawn / pty:exit) and serves
...
// read-write (the Harness owns PTY events) instead of read-only. Runs only in
// the plain-Node Harness process, so better-sqlite3 uses its standard binding.
...
import {
  ...
  type EventLogSqlite,
} from "../../shared/src/event-log";
import type { CoreLinkEvent } from "../../shared/src/core-link-frames";
