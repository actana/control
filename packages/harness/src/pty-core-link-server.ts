...
// frames to a `PtyHarnessCore`. The Harness process (harness-runner.mjs) loads
...
// server module alongside {@link HarnessQueryPort} (the per-Core navigation
// query port, issue 07).
export type { CoreLinkProjectSnapshot, CoreLinkTaskSnapshot };
import type { PtyHarnessCore } from "./pty-manager";
...
/**
 * Read-only access to the Harness's project + task tables for the per-Core
 * navigation + Fleet view (issue 07, ADR 0001). The Harness is the single
 ...
 * them.
 *
 * The real implementation (harness-query-store.ts) reads the shared
 * SQLite read-only; tests inject an in-memory fake. When omitted, the server
 * answers both frames with empty results (backward compat — a Harness with no
 * query port wired is still a valid PTY-only Core).
 */
export interface HarnessQueryPort {
  /** Every project on this Harness, as a flattened snapshot. */
  listProjects(): CoreLinkProjectSnapshot[];
  /**
   * Every task on this Harness (optionally filtered to one project). The
   * Harness omits archived tasks — the Fleet view is for active work, and the
...
/**
 * Read-write access to the Harness's projects + tasks tables for the
 * `projectsMutate` / `tasksMutate` / `sessionsList` core-link frames (issue
 * 04, ADR 0004). The Harness process owns the write path against its SQLite;
 * on a remote VM no sibling stateful server runs, so mutations go through
 * this port directly. Path validation for projects lives on the Harness (a
 ...
 *
 * The real implementation (harness-mutation-store.ts) opens the
 ...
 * `[]` — the same "PTY-only Harness is a valid Core" backward compat the
 * query port has.
 */
/**
 * Snapshot access to the Harness's CLI availability map (issue 11). The store
 ...
 * request. When omitted (a Harness with no availability probe wired, or a
...
/**
 * The Harness's own filesystem, as the Panel's folder picker browses it
...
export interface HarnessMutationPort {
  ...
  /**
   * Every active session on this Harness (optionally filtered to one
   * project). A session is a task-plus-optional-live-PTY: `ptyId` is set
   * when the Harness's PTY core currently has a running PTY for that task,
...
export type PtyCoreLinkServerOptions = {
  ...
  /**
   * The per-Harness event log. When provided, PTY lifecycle events are
   ...
   * builds a `wss://` server: the Harness presents `serverCert`/`serverKey`,
   ...
   * (a PTY-only Harness is still a valid Core).
   */
  queryPort?: HarnessQueryPort;
  /**
   ...
   * stubs so a PTY-only Harness (or a test) still round-trips the frames.
   */
  mutationPort?: HarnessMutationPort;
  /**
   ...
   */
  directoryPort?: HarnessDirectoryPort;
  /**
   ...
   * stand up a Harness that has drifted — the Panel's version gate has nothing
 ...
 * or `{ ok: false, reason }` matching `authError.reason`. The Harness wires
...
/**
 * mTLS material for the core-link server (issue 04). The Harness presents
...
/**
 * Heartbeat cadence, mirroring the Panel-side client. The Harness must detect a
...
/**
 * Hosts the loopback core-link WebSocket server. One instance per Harness
 * process. The server outlives individual Panel connections — PTY state is
 * retained in the `PtyHarnessCore` across disconnects/reconnects.
 *
...
export class PtyCoreLinkServer {
  ...
  private readonly queryPort: HarnessQueryPort | null;
  private readonly mutationPort: HarnessMutationPort | null;
  private readonly availabilityPort: HarnessAvailabilityPort | null;
  private readonly directoryPort: HarnessDirectoryPort | null;
  private readonly protocolVersion: string;
  private activeWs: WebSocketLike | null = null;
  private connection: ActiveConnection | null = null;

  constructor(
    private readonly core: PtyHarnessCore,
   ...
   * A Harness wired without it is still a valid Core; the Panel's typed-path
   * fallback keeps working, since project paths are validated on create.
   */
  private requireDirectoryPort(ws: WebSocketLike, reqId: string): HarnessDirectoryPort | null {
  ...
  private async dispatch(ws: WebSocketLike, frame: CoreLinkRequestFrame): Promise<void> {
    switch (frame.type) {
      ...
      // wiring). Task + project ops delegate to the HarnessQueryPort so the
      // Panel renders live snapshots with no Panel-side persistence. When no
      // queryPort is configured (a PTY-only Harness, or tests), both answer
...
class ActiveConnection {
  ...
  startPoll(fn: () => void, intervalMs: number): void {
    ...
    // Unref so the timer never keeps the Harness process alive on shutdown.
