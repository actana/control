# Actana Control — Product Spec (v1)

> **Superseded in part (2026-08-01).** This spec describes the original
> Electron desktop app. ADR 0010 replaced that shell with a self-hosted web
> service: the Panel runs as one deployable and is driven from a browser, the
> loopback Core is gone, and the Electron IPC surface below no longer exists.
> The product goals still hold; the delivery mechanism does not. See
> [`docs/adr/0010-panel-becomes-a-self-hosted-web-service.md`](docs/adr/0010-panel-becomes-a-self-hosted-web-service.md)
> and the current [README](README.md).
...
- Single Actana Control grid view of project cards with at-a-glance status (running / needs-input / done counts)
...
| -------------------- | --------------------- | ----------- | ------------------------------------------------------------------------ |
| `/`                  | Actana Control       | none (UI)   | Grid of all project cards                                                |
...
- **Key UI patterns:**
  ...
  - Top bar always shows a clickable `Actana Control` logo crumb that returns to home
...
- [x] **T2: Electron + TanStack Start integration** — Electron main spawns a TanStack Start server on `127.0.0.1:<random-port>`, BrowserWindow loads it. Verify dev mode (Vite HMR) works and prod build works.
  ...
  - Complexity: L
  - Dependencies: T1
  - Status: Dev mode done — Electron waits on `wait-on tcp:5173` then loads `AC_DEV_URL`. Prod build path (`pnpm package`) wired but unverified.
...
- [x] **T6: Actana Control view** — port `MissionControl` + `ProjectCard` + `Section` + `EmptyState` from `designs/views.jsx`. Wire up density toggle, search, group/pin sections, dot-grid background. Hook up to live data via TanStack Query (loaders) hitting the projects API.
