#!/usr/bin/env bash
# First-boot Core install for the dev Core-in-a-box (see the unit file for
...
# --no-harnesses: vendor CLI installers need the network and logins; a dev
...
machinectl shell --quiet operator@ /bin/bash -lc '
  set -euo pipefail
  cd ~
  tarball=$(ls -t /opt/core/actana-core-*.tar.gz | head -1)
  tar -xzf "$tarball"
  dir=$(basename "$tarball" .tar.gz)
  "./$dir/bin/actana" setup --public-host core --yes --no-harnesses 2>&1 | tee setup-output.txt
  # The blob is the one long base64 line of the output (same structural
  # extraction as scripts/lib/core-smoke.mjs).
  grep -E "^[A-Za-z0-9+/=]{100,}$" setup-output.txt | tail -1 > registration-blob.txt
  test -s registration-blob.txt
'

touch /home/operator/.core-provisioned
