...
How Actana Control knows whether a harness terminal is **working**, **idle**,
...
background subagent's completion re-invokes the main harness, and *that* turn's
...
- A held `Stop` (and a recent-finish heal) arms a once-a-minute recheck. Once
  ...
  3-minute drain grace starts: if the re-invoked main harness's own `Stop` lands
...
2. Passes them as `mcEnv` plus the harness type to `pty:spawn`.

The Electron main process (`electron/pty-manager.ts`) calls
`installHarnessHooks(harness, cwd)` (`electron/harness-hooks.ts`), which uses a small
per-harness hook registry:
...
endpoint `POST /api/hooks/<harness-slug>?taskId=<id>` (`src/server/api-router.ts`)
...
Hooks only fire while the harness process lives, so two out-of-band signals
...
the harness could call. It works, but:
