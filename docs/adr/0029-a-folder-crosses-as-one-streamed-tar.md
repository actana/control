...
**D6a — Replacing a file is not the same promise as removing a tree.** An entry — or a single-file `PUT` — landing on a path that currently holds a **non-empty directory** is refused, with its own code (`directory-in-the-way`), rather than performed.

D6 is about the *final component* and about symlinks, and overwrite-by-default ([#129](https://github.com/actana/control/issues/129) F5) is about replacing a file. Neither says anything about deleting a subtree, and the first implementation of this ADR quietly did: a `PUT ?path=src` meant for `src/x.ts` removed `src` recursively and reported it as an ordinary `overwritten` line, so the damage was both larger than asked for and invisible in the progress stream. `tar(1)` refuses this case rather than deleting, and the refusal is the honest answer — the operator can remove the directory themselves if that is genuinely what they meant. An **empty** directory is still replaced: there is nothing to lose, and a stray `mkdir` should not wedge a path forever.

**D6b — A size the ustar field cannot hold is written as a pax `size` record, not refused.** The ustar size field is 11 octal digits, which stops at 8589934591 bytes — one byte under 8 GiB. F8 says there is no size cap, so there is not one: the packer emits a pax `size` record above that bound and writes 0 in the ustar field, and the unpacker reads both a pax `size` record and the GNU base-256 encoding that other writers use.

Worth recording because the alternative was already in the code as a *comment* — `writeOctal` claimed `packEntryHeader` fell back to a pax record — while `packEntryHeader` only ever did so for a long name or link target. The gap failed closed in both directions (a mid-stream `corrupt-archive` on pack, an empty file and a desynchronised stream on unpack), which is exactly why it could sit there unnoticed: nothing was wrong except that a documented promise was not kept, at a magnitude no test fixture reaches.
