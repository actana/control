# Linux Harness — the one-liner rehearsal
...
[the macOS checklist](harness-macos-prerelease-checklist.md), which covers the
...
On **macOS**, `pnpm harness:tarball` builds a `mac-*` tarball — the bundled
...
That writes into `artifacts/harness/` like the native build does, and
`harness:rehearse` picks up the newest tarball for your architecture on its own.
...
- [ ] It offers to install the agent CLIs it found missing, one at a time.
...
- [ ] `Harness: healthy`, the version you just built, and a `Linger` row.
