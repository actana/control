# macOS Harness — manual pre-release checklist
...
Its Linux counterpart is [the one-liner rehearsal](harness-linux-rehearsal.md) —
`pnpm harness:rehearse` for a throwaway machine to paste the real one-liner
...
- No Actana Core installed yet (`launchctl print gui/$(id -u)/com.actana.harness`
...
```bash
tar -xzf actana-harness-<version>-mac-arm64.tar.gz && ./actana-harness-<version>-mac-arm64/bin/actana setup
```
...
- [ ] `Harness: healthy`, the LaunchAgent named as `com.actana.harness`, and an
      `At login` row.
...
Log back in as the same user. **Wait 30 seconds** (launchd starts agents a
...
- [ ] `Harness: healthy` again, with a **different** PID from before the reboot.
...
- [ ] `launchctl print gui/$(id -u)/com.actana.harness` reports no such service.
- [ ] `~/Library/LaunchAgents/com.actana.harness.plist`, `~/.local/bin/actana`,
