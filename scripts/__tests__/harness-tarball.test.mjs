...
describe("HARNESS_TARGETS", () => {
  it("covers exactly the four supported Cores", () => {
    expect(HARNESS_TARGETS.map((t) => t.target)).toEqual([
      "mac-arm64",
      "mac-x64",
      "linux-x64",
      "linux-arm64",
  ...
  it("carries no Windows target", () => {
    expect(HARNESS_TARGETS.some((t) => t.platform === "win32")).toBe(false);
  });
});
...
describe("HARNESS_RUNTIME_DEPENDENCIES", () => {
  it("covers every external the Harness build leaves unbundled", () => {
    // Adding an `external` to packages/harness/build.mjs without adding it
    // here ships a tarball that dies on its first require(). Fail here instead.
    const buildScript = fs.readFileSync(
      path.join(repoRoot, "packages", "harness", "build.mjs"),
      "utf8",
    );
    ...
    const missing = externals.filter(
      (name) => !HARNESS_RUNTIME_DEPENDENCIES.includes(name) && !UNBUNDLED_EXTERNALS.includes(name),
    );
...
describe("formatShasums", () => {
  it("renders digest, two spaces, name — sorted by name", () => {
    const text = formatShasums(
      new Map([
        ["actana-harness-1.0.0-mac-x64.tar.gz", "b".repeat(64)],
        ["actana-harness-1.0.0-linux-x64.tar.gz", "a".repeat(64)],
      ]),
    );

    expect(text).toBe(
      `${"a".repeat(64)}  actana-harness-1.0.0-linux-x64.tar.gz\n` +
        `${"b".repeat(64)}  actana-harness-1.0.0-mac-x64.tar.gz\n`,
    );
  });
...
describe("tarball naming", () => {
  it("names the archive after version and target", () => {
    expect(tarballName("0.49.0", "mac-arm64")).toBe("actana-harness-0.49.0-mac-arm64.tar.gz");
  });

  it("puts everything under one directory so extraction never litters the CWD", () => {
    expect(tarballRootDirName("0.49.0", "mac-arm64")).toBe("actana-harness-0.49.0-mac-arm64");
  });
});
...
describe("planDependencyLayout", () => {
  ...
  it("resolves each dependency from the package that declares it", () => {
    ...
    // own directory, never from the harness package.
