#!/usr/bin/env node
// Smoke test — a released Harness tarball is self-sufficient.
//
...
// standalone smoke makes (see scripts/lib/harness-smoke.mjs).
//
...
// launcher boots a dialable Harness with no system Node." If a future change
...
// and the build is what guarantees it: everything the Harness loads is copied
// into the tarball, and nothing in the boot path fetches.
//
// Usage:
//   node scripts/smoke-harness-tarball.mjs --tarball <file> [--timeout <ms>]
//
// --tarball <file>  The .tar.gz produced by scripts/build-harness-tarball.mjs.
...
function readManifest(installRoot) {
  const manifestPath = path.join(installRoot, "harness-manifest.json");
  if (!fs.existsSync(manifestPath)) die("tarball has no harness-manifest.json at its root");

  const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
  for (const field of ["version", "protocolVersion", "target", "nodeVersion"]) {
    if (typeof manifest[field] !== "string" || manifest[field] === "") {
      die(`harness-manifest.json is missing ${field}`);
    }
  }
...
async function main() {
  ...
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "actana-harness-smoke-"));
  ...
  log("OK — extracted tarball boots a dialable Harness with no system Node");
  cleanup();
  process.exit(0);
}
