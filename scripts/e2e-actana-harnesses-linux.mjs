#!/usr/bin/env node
// End-to-end test — Harness detection and offers on a fresh systemd machine.
//
// This is issue 05's acceptance criteria run against a machine that genuinely
// has no Harness on it, which is the one thing no unit test can supply:
//
//   • a fresh machine has none of them, `--no-harnesses` installs none, and a run
//     with no terminal neither prompts nor installs;
//   • `actana harnesses install <id>` runs the vendor's own installer and leaves
//     a working CLI on PATH; an unknown id fails with the supported list;
//   • the Core reflects the new CLI with no restart — the daemon's pid is
//     unchanged and a test client dialling the core-link sees it `available`,
//     which is exactly what a paired Panel would see;
//   • `--with-<harness>` installs unattended, with no terminal anywhere;
...
//
// Usage:
//   node scripts/e2e-actana-harnesses-linux.mjs --tarball <file> [--distro <id>] [--keep]
//
...
/** How long the Core gets to notice a newly installed CLI. */
...
async function main() {
  ...
  const machine = await startSystemdContainer({
    tag: imageTag("harnesses", distro.id),
  ...
  const mainPid = () =>
    mustAsOperator(
      "systemctl --user show actana-core.service --property=MainPID --value",
    ).stdout.trim();
  ...
  log("fresh machine: no Harness on PATH");
  ...
  if (!/actana harnesses install/.test(piped.stdout)) {
  ...
  const unknown = asOperator("actana harnesses install gemini");
  if (unknown.status !== 2) {
  ...
  // ─── `actana harnesses install` runs the real vendor installer ───
  ...
  const installed = mustAsOperator("actana harnesses install opencode");
  if (!installed.stdout.includes("Installing OpenCode")) {
    die("`actana harnesses install` did not say what it was running", installed.stdout.split("\n"));
  }
  if (!onPath("opencode")) {
  ...
  if (/^Install /m.test(withFlag.stdout)) {
    die("--with-<harness> prompted instead of installing", withFlag.stdout.split("\n"));
  }
  if (!onPath("claude")) {
  ...
  log("`--with-<harness>` installed exactly that agent, unattended");
  ...
  log("the Core's published availability matches what was installed");

  log(
    `OK — agent detection, offers, and \`actana harnesses install\` work on a fresh ${distro.label} machine`,
  );
  process.exit(0);
}
