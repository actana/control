// Pure helpers for the per-platform Harness tarball build.
//
// Everything here is side-effect free so `scripts/__tests__/harness-tarball.test.mjs`
...
//
// The I/O that uses these lives in scripts/build-harness-tarball.mjs and
// scripts/compose-harness-shasums.mjs.

import path from "node:path";

/**
 * Node runtime bundled into every Harness tarball. Kept in lockstep with the
 * `node-version` the CI workflows install — a Harness that ships a different
 * Node than the one its native modules were built against will not load them.
 */
export const BUNDLED_NODE_VERSION = "24.15.0";

/**
 * The four Harness release targets. No Windows Core: Windows operators run the
 ...
 * cross-compiled).
 */
export const HARNESS_TARGETS = Object.freeze([
...
/**
 * Packages the Harness `require()`s at runtime rather than bundling.
 *
 * Must cover every `external` in `packages/harness/build.mjs` that the Harness
 ...
 */
export const HARNESS_RUNTIME_DEPENDENCIES = Object.freeze([
  "better-sqlite3",
  "node-pty",
  "ws",
  "selfsigned",
...
/** Externals in the Harness build that deliberately do not ship in the tarball. */
...
# actana — Harness launcher. Execs the bundled Node runtime on the bundled
...
# The Harness resolves bundled resources relative to its app path.
 ...
 * packages the Harness never `require()`s. The tarball ships the already-built
 ...
 * most of the weight. The tarball smoke (`scripts/smoke-harness-tarball.mjs`)
...
/**
 * The `harness-manifest.json` written at the tarball root.
 *
