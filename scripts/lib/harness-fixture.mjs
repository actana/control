// The Harness fixture the Panel e2e pairs with.
//
...
//
//   • `startLocalHarness` boots the built Harness as a plain child process.
//     It is what a macOS runner with no Docker can do, and it is fast.
//   • `startContainerHarness` is the spec's canonical "Core-in-a-box": the
...
/** The port the Harness listens on inside a Core-in-a-box. */
...
/**
 * Boot a Harness as a local process against a throwaway HOME and data
 * directory, and hand back the fixture interface above.
 *
 * The blob's endpoint is rewritten to the loopback address the Harness
 * actually bound. The Harness mints its server cert with a `127.0.0.1` SAN
 * (`AC_HARNESS_PUBLIC_HOST`), so this is the address the Panel's pinned-CA
 * dial verifies against — the same rewrite the tarball smoke makes.
 */
export async function startLocalHarness({ entry, timeoutMs = DEFAULT_BOOT_TIMEOUT_MS, log } = {}) {
  if (!entry || !fs.existsSync(entry)) {
    throw new Error(
      `harness entry not found at ${entry} — run \`pnpm --filter @actana/harness build\` first`,
    );
  }
  const port = await pickFreePort();
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "ac-e2e-harness-"));
  ...
  try {
    ...
    if (!observer.blob) throw new Error("harness printed no registration blob");
  } catch (err) {
    stop();
  ...
  log?.(`harness fixture up on ${endpoint} (home=${home})`);
 ...
 * about the fixture reaches past `actana` into the Harness, so what the Panel
 ...
 * reason `startLocalHarness` rewrites it: setup mints the server cert with a
 ...
 */
export async function startContainerHarness({
  tarball,
  ...
  if (!tarball || !fs.existsSync(tarball)) {
    throw new Error(
      `no tarball at ${tarball} — run \`pnpm harness:tarball\` first, or drop the --harness-tarball flag`,
    );
  }
  ...
  // `--no-agents`: what this fixture is for is a Core the Panel can pair with
  // and drive. Agent CLIs are a separate seam with its own e2e, and attempting
  // their vendor installers here would make a hermetic fixture depend on the
  // network.
  const setup = machine.asOperator(
    `cd ~ && tar -xzf ${asset} && ${extracted}/bin/actana setup --public-host 127.0.0.1 --yes --no-agents`,
  );
