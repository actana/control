#!/usr/bin/env node
// Smoke test — the standalone Harness boots clean under plain node.
//
// Boots packages/harness/dist/harness-entry.cjs with the current node binary
// (standard Node ABI) against a temp HOME,
// waits for the @@AC_HARNESS_LISTENING@@ marker, then dials the core-link
...
// future change ships a Harness that can't load its natives on the normal
...
// `scripts/smoke-harness-tarball.mjs`; the shared sequence lives in
// `scripts/lib/harness-smoke.mjs`.
//
// Usage:
//   node scripts/smoke-standalone-harness.mjs [--entry <file>] [--port <n>] [--timeout <ms>]
//
// --entry <file>    Harness bundle (default: packages/harness/dist/harness-entry.cjs;
//                   run `pnpm --filter @actana/harness build` first).
...
async function main() {
  ...
  if (!fs.existsSync(entry)) {
    die(`harness entry not found at ${entry} — run \`pnpm --filter @actana/harness build\` first`);
  }

  // Isolate every side-effect the Harness might have to a fresh temp dir:
  ...
  log("OK — standalone Harness boots clean under plain node");
  cleanup();
  process.exit(0);
}
