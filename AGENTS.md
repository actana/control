...
Issues and specs live as GitHub issues on `actana/control`, driven with the `gh` CLI. See `.agents/issue-tracker.md`.

### Triage labels

Five canonical roles as label strings: `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`. See `.agents/triage-labels.md`.
