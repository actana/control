...
Product backlog for Actana Control — personal + work use cases.

---

## Metrics & token usage

The Cursor status line shows live session token usage, but Actana Control still lacks easy at-a-glance totals per project and per session, plus cost/time KPIs for repeatable work.
...
- [ ] **Per-project token totals on Actana Control grid** — show cumulative usage on each `ProjectCard` (badge or subtitle), drill-down to project-scoped usage
...
- [ ] **Multi-harness usage ingestion** — extend beyond Claude Code JSONL to Codex and Cursor CLI usage logs where available
...
Idea: n8n-style visual task flows for repeatable harness workflows — define, visualize, and run consistently. May overlap with GitHub/GitLab/Codeberg/Forgejo CI; evaluate whether the overhead is worth it vs. delegating to existing CI.
...
- [ ] **Step types (MVP)** — shell command, harness prompt, wait-for-status, conditional branch
...
On Windows, copying a row in Claude (terminal selection / context copy) does not copy to clipboard; paste also fails. Re-triage needed: the clipboard path is now the browser's async Clipboard API (`attachTerminalKeyHandler`), not an IPC bridge, so the original diagnosis no longer applies.

- [ ] **Re-reproduce on Windows** — a Chromium browser against a paired Windows Core
- [ ] **Clipboard permissions** — confirm the Panel's origin is granted `clipboard-read`; a denial currently swallows the paste silently
- [ ] **Regression test** — manual QA checklist item for Windows copy/paste in harness terminals
...
| Terminal / clipboard | `packages/panel/src/components/views/TerminalPane.tsx`, `packages/panel/src/lib/terminal-pane-helpers.ts` |
